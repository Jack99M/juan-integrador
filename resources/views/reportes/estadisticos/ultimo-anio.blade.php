...
<head>
    <meta charset="utf-8">
    <title>Reporte Último Año</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .period { text-align: center; background: #f8d7da; padding: 15px; border-radius: 8px; margin: 15px 0; }
        ...
        .section-title { font-size: 18px; color: #dc3545; border-bottom: 2px solid #dc3545; padding-bottom: 8px; margin: 20px 0 15px; }
        table { width: 100%; border-collapse: collapse; margin: 15px 0; }
        th { background: #dc3545; color: white; padding: 10px; text-align: left; font-size: 12px; }
...
<body>
    <div class="header">
        <h1>REPORTE ÚLTIMO AÑO</h1>
        <div class="subtitle">Generado el {{ date('d/m/Y H:i:s') }}</div>
    </div>

    <div class="container">
        <div class="period">
            <strong>Periodo:</strong> {{ $fechaInicio->format('d/m/Y') }} - {{ now()->format('d/m/Y') }}
        </div>
        ...
        <div class="section-title">Imágenes Analizadas (Mostrando últimas 50)</div>
        <table>
            <thead>
                <tr>
                    <th>Código</th>
            ...
            <tbody>
                @foreach($imagenes->take(50) as $imagen)
                <tr>
        ...
        @if($imagenes->count() > 50)
        <p style="text-align: center; color: #666; margin-top: 15px;">
            <em>Mostrando 50 de {{ $imagenes->count() }} resultados</em>
        </p>
        @endif
    </div>
